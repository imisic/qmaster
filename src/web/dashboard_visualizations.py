...
class DashboardVisualizer:
    ...
    def _read_metadata(metadata_file: Path) -> dict[str, Any] | None:
        ...
        try:
            ...
            logging.debug("Failed to read metadata %s: %s", metadata_file.name, e)
            return None
                ...
                for line in f:
                    ...
                    if " - BackupEngine - " in line:
                        try:
                            ...
                            logging.debug("Skipped unparseable log line: %s", e)
                            continue
        except Exception as e:
            logging.warning("Error reading backup success rate: %s", e)
            return None
            ...
            for item_dir in category_dir.iterdir():
                ...
                for metadata_file in item_dir.glob("*.json"):
                    ...
                    try:
                        ...
                        logging.debug("Skipped unparseable metadata timestamp: %s", e)
                        continue
