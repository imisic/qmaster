...
- Found: `config_manager.py:135-142` — `get_all_databases()` calls `.copy()` on outer dict, then mutates inner dicts by decrypting passwords. After first call, in-memory config holds plaintext passwords.
- Fix: use `copy.deepcopy()` or build a new dict with decrypted values without mutating originals
...
- Found: `engine.py:188-192` — symlink creation without locking; concurrent backups race on `latest.tar.gz` symlink
